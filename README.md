...
- Executor pipeline updated to `LLM -> TTS -> audio message with translated caption`
...
### ✅ Iteration 06
- OpenTelemetry metrics integration added
- OTLP exporter configured via `ObservabilityOptions`
- Structured observability logging events added across webhook/worker/LLM/TTS/TelegramSender
- Metrics added for jobs, worker, LLM, TTS, and Telegram sending
...
- [x] Iteration 06 — Observability (OpenTelemetry)
...
- `UpdatePayload` is stored as `TEXT` (raw JSON string), not `json/jsonb`
...
Current transport API for audio:
